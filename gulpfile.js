...
gulp.task('htmlProd', function() {
  return gulp
    .src(['./src/[^_]*.html', './src/*/[^_]*.html', '!./src/temp/**'])
    .pipe(
      nunjucks.compile({
        build: 'prod'
      })
...
gulp.task('js', function() {
  ...
  return gulp
    .src(['./src/js/config-dev.js', './src/js/scripts.js'])
...
gulp.task('jsBabel', function() {
  return gulp
    .src(['./src/js/config-prod.js', './src/js/scripts.js'])
    .pipe(
      babel({
        presets: ['@babel/env']
      })
...
gulp.task('buildJs', function() {
  return gulp
    .src(['./dist/js/libs.js', './dist/js/script.js'])
    .pipe(concat('scripts.js'))
    .pipe(gulp.dest('./dist/js'));
});
...
gulp.task('fonts', function() {
  return gulp.src('./src/fonts/**').pipe(gulp.dest('./src/temp/fonts'));
});
  ...
  chokidar
    ...
    .on('all', (event, path) => {
      if (event === 'add') {
        ...
        newPath = ('./src/temp' + path.substring(3)).substring(
          0,
          newPath.lastIndexOf('/')
        );
...
gulp.task('copyOtherFiles', function() {
  return gulp.src('./src/*.@(php|mp4|webm|pdf)').pipe(gulp.dest('./dist'));
});

gulp.task('copyOtherFilesTemp', function() {
  return gulp.src('./src/**/*.@(php|mp4|webm|pdf)').pipe(gulp.dest('./src/temp'));
});
